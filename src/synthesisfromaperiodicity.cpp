...
void GetAperiodicResponse(int noise_size, int fft_size,
    ...
    double *fractional_f0_phase, ForwardRealFFT *forward_real_fft,
  ...
  for (int i = 0; i <= fft_size / 2; ++i) {
    inverse_real_fft->spectrum[i][0] =
      ...
      cos(fractional_f0_phase[i]) +
      minimum_phase->minimum_phase_spectrum[i][1] *
      sin(fractional_f0_phase[i]);
...
void GetPeriodicResponse(int fft_size, double *spectrum,
    double *aperiodic_ratio, double current_vuv, double *fractional_f0_phase,
    InverseRealFFT *inverse_real_fft, MinimumPhaseAnalysis *minimum_phase,
  ...
  for (int i = 0; i <= minimum_phase->fft_size / 2; ++i)
    minimum_phase->log_spectrum[i] =
      log(spectrum[i] * (1.0 - aperiodic_ratio[i])) / 2.0;
  GetMinimumPhaseSpectrum(minimum_phase);
...
//-----------------------------------------------------------------------------
void GetOneFrameSegment(double current_f0, double current_vuv, int noise_size,
    double **spectrogram, int fft_size, double **aperiodicity, int f0_length,
    double frame_period, double current_time, int fs, double fractional_index,
  ...
  double *fractional_f0_phase = new double[fft_size / 2 + 1];
  for (int i = 0; i <= fft_size / 2; ++i)
    fractional_f0_phase[i] =
      2.0 * world::kPi * fractional_index * i / fft_size;

  // Synthesis of the periodic response
  GetPeriodicResponse(fft_size, spectral_envelope, aperiodic_ratio,
    current_vuv, fractional_f0_phase, inverse_real_fft, minimum_phase,
      periodic_response);

  // Synthesis of the aperiodic response
  GetAperiodicResponse(noise_size, fft_size, spectral_envelope,
      aperiodic_ratio, current_vuv, fractional_f0_phase, forward_real_fft,
...
int GetPulseLocationsForTimeBase(double *interpolated_f0, double *time_axis,
    ...
    int *pulse_locations_index, double *fractional_index) {
...
int GetTimeBase(double *f0, int f0_length, int fs,
    ...
    int *pulse_locations_index, double *fractional_index,
    double *interpolated_vuv, double *interpolated_f0) {
...
DLLEXPORT void SynthesisFromAperiodicity(double *f0, int f0_length,
  ...
  int number_of_pulses = GetTimeBase(f0, f0_length, fs, frame_period / 1000.0,
      y_length, pulse_locations, pulse_locations_index, fractional_index,
      interpolated_vuv, interpolated_f0);
  ...
  for (int i = 0; i < number_of_pulses; ++i) {
    ...
    GetOneFrameSegment(interpolated_f0[pulse_locations_index[i]],
        interpolated_vuv[pulse_locations_index[i]], noise_size, spectrogram,
        fft_size, aperiodicity, f0_length, frame_period, pulse_locations[i],
        fs, fractional_index[i], &forward_real_fft, &inverse_real_fft,
        &minimum_phase, impulse_response);

    int index = 0;
    for (int j = 0; j < fft_size; ++j) {
      index = MyMin(y_length - 1,
        MyMax(0, j + pulse_locations_index[i] - fft_size / 2));
