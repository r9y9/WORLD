...
void F0Estimation(double *x, int x_length, int fs, int f0_length, double *f0,
  ...
  std::ofstream f0rstream("f0_refined.txt");
  f0rstream.precision(16);
  for (int i = 0; i < f0_length; ++i) {
    f0rstream << f0[i] << std::endl;
  }
...
int main(int argc, char *argv[]) {
  ...
  std::ofstream xstream("x.txt");
  xstream.precision(16);
  for (int i = 0; i < x_length; ++i) {
    xstream << x[i] << std::endl;
  }
